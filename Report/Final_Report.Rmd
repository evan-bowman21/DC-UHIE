...
model_best <- linear_reg() %>%
  fit(HSI ~ P_POC + P_CHILD + P_ELDERLY + P_POVERTY + P_LIMENG + OBESITY, data = hsi_train)

model_backward <- linear_reg() %>%
  fit(HSI ~ . - OBESITY, data = hsi_train)
...
out_preds_reg <- bind_cols(out_preds_reg, 
                           predict(model_backward, new_data = hsi_test)) %>%
                           rename(backward = .pred)

out_metrics_reg <- out_preds_reg %>% metrics(truth = HSI, estimate = best) %>%
     rename(best = .estimate) %>%
  left_join(out_preds_reg %>% metrics(truth = HSI, estimate = backward) %>%
            rename(backward = .estimate))
...
```{r, echo=FALSE}
best_lm <- lm(HSI ~ . - OBESITY, data = hsi_model)
