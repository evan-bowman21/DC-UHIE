...
knitr::kable(mean.cv.error, col.names = c("Variables", "CV Error"))

```

The model including ten variables was identified as the lowest mean RMSE across the ten folds. This model was then fitted with the whole data for coefficient identification (Table ?).

```{r, echo=FALSE, warning=FALSE, out.height="60%", out.width="60%"}
# We then perform best subsets selection on the full data set in order to obtain the 10-variable model:
reg.best <- regsubsets(HSI ~ ., data=hsi_model, nvmax=13)
best_coeff <- as.table(coef(reg.best,10))

knitr::kable(best_coeff, col.names = c("Predictor", "Coefficient"), label = "Fig ?")
```

### Shrinkage Regression

While best subsets regression identified the best model as including 10 variables, there is clear covariation that affects the magnitude of the coefficients (Appendix Tab. 3). As a result, a LASSO regression model will be fitted and compared to the original regression model. LASSO takes into account the covariation of the predictors, and adjusts the coefficients accordingly. The LASSO model was tuned using K-fold cross validation.. 
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.height="60%", out.width="60%"}
library(glmnet)
set.seed(2200)
...
Also, by obtaining the MSE of the two lambda values we can compute the RMSE and compare to that of the best subsets method. The comparison of the RMSE of the three models highlights that the best subsets model has the best predictive power. This model, while not generative, offers could predictors of socioeconomic factors that are correlated with HSI levels for census tracts. 
```{r, echo= FALSE, out.height="60%", out.width="60%"}
rmse <- cbind(best_rmse, lambda_min_rmse, lambda_1se_rmse)
rmse <- as.table(rmse)
colnames(rmse) <- c("Best", "Lambda Min", "Lambda 1se")
rownames(rmse) <- "RMSE"

knitr::kable(rmse, label  = "Table ?")
```
...
```{r, echo=FALSE, out.height="60%", out.width="60%"}
plot(cv.lasso.mod, sub = "Appendix Fig. 2: Lambda Values of K-Fold CV LASSO")
```
