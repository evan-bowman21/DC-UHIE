...
Model 10 returned the  highest adjusted r-squared value, lowest Mallow's Cp value, and lowest BIC value. This shows that the model captures one of the the highest variations in HSI, the best predicting power for all possible models, and the lowest bias for all possible models using the Best Subsets method. Furthermore, the Mallow's CP value is near the number of predictors of the model. This further shows the lack of bias the model has. 

The top model from each the best subsets method was trained and validated to understand which the true predicting power. 
...
regfit.best <- regsubsets(HSI ~ ., data = hsi_train, nvmax = 13)

# Compute the validation set error for the best model of each model size.
val.mat <- model.matrix(HSI ~ ., data = hsi_test) # This a model matrix from the validation data.
head(val.mat)
val.errors <- rep(NA, 13) # there are 13 variables and we will look at a model of each size (i.e num vars)
for (i in 1:13) {
 ...
 val.errors[i] <- sqrt(mean((hsi_test$HSI - pred)^2)) # RMSE square root of (validation observed y - yhat)^2
}
data.frame(val.errors,numvars=1:13)
which.min(val.errors)
coef(regfit.best,id=which.min(val.errors))
best_rmse <- val.errors[12]
```

Model fit to the whole data set.
```{r, echo=FALSE, warning=FALSE}
regfit.best <- regsubsets(HSI ~ ., data = hsi_test, nvmax = 13)
best_coeff <- coef(regfit.best,12)
knitr::kable(best_coeff)
```

### Shrinkage Regression

While best subsets regression identified the best model as including 10 variables, there is clear covariation that affects the magnitude of the coefficients (Appendix Tab. 3). As a result, a LASSO regression model will be fitted and compared to the original regression model. LASSO takes into account the covariation of the predictors, and adjusts the coefficients accordingly. 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
library(glmnet)
# LASSO 
train_x <- model.matrix(HSI ~ ., hsi_train)[, -10]
train_y <- hsi_train$HSI

test_x <- model.matrix(HSI ~ ., hsi_test)[, -10]
test_y <- hsi_test$HSI
set.seed(2200)
...
set.seed(1)
cv.lasso.mod <- cv.glmnet(train_x, train_y, alpha = 1)
plot(cv.lasso.mod)
cv.lasso.mod$lambda.min

lasso_coeff <- predict(lasso.mod, s=cv.lasso.mod$lambda.min, type="coefficients" )
lasso_pred <- predict(lasso.mod, s=cv.lasso.mod$lambda.min, newx = test_x, type = "response")
...
```{r, echo=FALSE}
rmse<- data.frame(best_rmse, lasso_rmse)
colnames(rmse) <- c("Best Subsets", "LASSO")
rownames(rmse) <- "RMSE"
...
```{r, echo=FALSE}
plot(1:13,val.errors,xlab='num vars',ylab='CV errors',main='CV errors vs. num vars', sub = "Appendix Fig.2", pch=19,type='b')
```
